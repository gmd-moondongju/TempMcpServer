...
namespace TempMcpServer
{
    ...
    {
        private const string McpTinyImage =
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            for (var i = 0; i < 100; ++i)
            {
                var uri = $"test://static/resource/{i + 1}";
                if (i % 2 == 0)
                {
                    resources.Add(
                        new Resource
    ...
    ""required"": [""prompt"", ""maxTokens""]
                ...
                Resources = new ResourcesCapability
                {
                    ListResourceTemplatesHandler = (_, _) => Task.FromResult(
                    ...
                    {
                        var startIndex = 0;
                        var requestParams = request.Params ?? new ListResourcesRequestParams();
                        if (requestParams.Cursor is not null)
                        {
                            try
                            {
                                var startIndexAsString =
                                    Encoding.UTF8.GetString(Convert.FromBase64String(requestParams.Cursor));
                        ...
                        var endIndex = Math.Min(startIndex + pageSize, resources.Count);
                    ...
                    ReadResourceHandler = (request, _) =>
                    {
                        ...
                        var contents = resourceContents.FirstOrDefault(r => r.Uri == request.Params.Uri) ??
                ...
                Prompts = new PromptsCapability
                {
                    ListPromptsHandler = (_, _) => Task.FromResult(
                    ...
                    GetPromptHandler = (request, _) =>
                    {
                        ...
                        switch (request.Params.Name)
                        {
                            ...
                            case "complex_prompt":
                            {
                                var temperature = request.Params.Arguments?["temperature"]?.ToString() ?? "unknown";
                                var style = request.Params.Arguments?["style"]?.ToString() ?? "unknown";
                                messages.Add(
                                    new PromptMessage
                                    {
                                        Role = Role.User,
