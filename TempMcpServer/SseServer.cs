...
namespace TempMcpServer
{
    ...
    {
        private static ILoggerFactory CreateLoggerFactory() => LoggerFactory.Create(builder => builder.AddDebug());

        public static async Task RunAsync(ILoggerFactory? loggerFactory = null, CancellationToken cancellationToken = default)
        {
    ...
    ""required"": [""prompt"", ""maxTokens""]
                    ...
                    CallToolHandler = async (request, cancellationToken) =>
                    {
                        ...
                        if (request.Params.Name == "echo")
                        {
                            ...
                            {
                                Content = new List<Content> { new() { Text = "Echo: " + message.ToString(), Type = "text" } }
                            };
                        }
                        else if (request.Params.Name == "sampleLLM")
                        {
                            ...
                            return new CallToolResponse()
                            {
                                Content = new List<Content> { new() { Text = $"LLM sampling result: {sampleResult.Content.Text}", Type = "text" } }
                            };
                        }
                        else
                        {
                ...
                Resources = new()
                {
                    ...
                    ReadResourceHandler = (request, cancellationToken) =>
                    {
                        ...
                        var contents = resourceContents.FirstOrDefault(r => r.Uri == request.Params.Uri) ?? throw new McpServerException("Resource not found");
                        return Task.FromResult(new ReadResourceResult()
                        {
